...
contract exchange{
    ...
    function getTokenBalance(address _owner, address _token) public view returns (uint){
        return tokenBalance[_owner][_token];
    }
    function getEthBalance(address _owner) public view returns (uint){
        return ethBalance[_owner];
    }
    ...
    function getBuyOrders(address _token) public view returns(uint[] memory, uint[] memory){
        ...
        uint buyPrice = loadedToken.maxBuyPrice;
        uint counter = 0;
        
        if (loadedToken.minBuyPrice>0){
            while(buyPrice>=loadedToken.minBuyPrice){
                ...
                if (buyPrice==loadedToken.buyBook[buyPrice].lowerPrice){
                    break;
                }else{
                    buyPrice=loadedToken.buyBook[buyPrice].lowerPrice;
                }
                counter++;
            }
        }
